...
(function($){
	...
	$.supersized = function( options ) {
		...
		var settings = {
			...
			image_path				:	'img/2011guo/img/', //Default image path
		...
		if (options.slides.length > 1){
			...
			$("<img/2011guo/img/>").attr("src", options.slides[loadPrev].image).appendTo(element).wrap('<a ' + imageLink + linkTarget + '></a>');
		}
		
		//Set current image
		imageLink = (options.slides[currentSlide].url) ? "href='" + options.slides[currentSlide].url + "'" : "";
		$("<img/2011guo/img/>").attr("src", options.slides[currentSlide].image).appendTo(element).wrap('<a class="activeslide" ' + imageLink + linkTarget + '></a>');
		...
		if (options.slides.length > 1){
			...
			$("<img/2011guo/img/>").attr("src", options.slides[loadNext].image).appendTo(element).wrap('<a ' + imageLink + linkTarget + '></a>');
		}
		...
		$(window).load(function(){
			...
			if (options.thumbnail_navigation){
				...
				$('#prevthumb').show().html($("<img/2011guo/img/>").attr("src", options.slides[prevThumb].image));
				
				//Load next thumbnail
				currentSlide == options.slides.length - 1 ? nextThumb = 0 : nextThumb = currentSlide + 1;
				$('#nextthumb').show().html($("<img/2011guo/img/>").attr("src", options.slides[nextThumb].image));
		...
		function resizenow() {
			return element.each(function() {
				...
				if (options.image_protect){
					...
					$('img/2011guo/img', element).bind("contextmenu",function(){
						return false;
					});
					$('img/2011guo/img', element).bind("mousedown",function(){
						return false;
					});
		...
		function nextslide() {
			...
			$("<img/2011guo/img/>").attr("src", options.slides[loadSlide].image).appendTo(element).wrap("<a " + imageLink + linkTarget + "></a>");	//Append new image
			...
			if (options.thumbnail_navigation == 1){
				...
				$('#prevthumb').html($("<img/2011guo/img/>").attr("src", options.slides[prevThumb].image));
			
				//Load next thumbnail
				nextThumb = loadSlide;
				$('#nextthumb').html($("<img/2011guo/img/>").attr("src", options.slides[nextThumb].image));
		...
		function prevslide() {
			...
			imageLink = (options.slides[loadSlide].url) ? "href='" + options.slides[loadSlide].url + "'" : "";	//If link exists, build it
			$("<img/2011guo/img/>").attr("src", options.slides[loadSlide].image).prependTo(element).wrap("<a " + imageLink + linkTarget + "></a>");	//Append new image
